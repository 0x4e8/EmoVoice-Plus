	...
	with open(pred_path, "w") as pred, open(gt_path, "w") as gt, open(question_path, "w") as q:
		for step, batch in enumerate(test_dataloader):
			...
			for i, key in enumerate(batch["keys"]):
				...
				# 修改代码
				emoSteerEnable = decode_config.emosteer.enable
				# print("DDDDDDDDDDDDDDDDDDDDDDDDDDDDDDDDDDDDDDDDDDDDD", emoSteerEnable)
				audio_hat = audio_decode_cosyvoice(audio_tokens, model_config, codec_decoder, audio_prompt_path, code_layer, num_latency_tokens, speed=1.0, decode_config=decode_config, emoSteerEnable=emoSteerEnable)
				if audio_hat == None:
