...
class SpeechDatasetJsonl(torch.utils.data.Dataset):
    ...
    def collator(self, samples):
        ...
        # prompt_end_pos = torch.tensor([
        #     (input_prompt_max_length - input_prompt_lengths[i]) + samples[i]["prompt_length"] - 1
        #     for i in range(len(samples))
        # ], dtype=torch.long)
        prompt_str_pos = torch.tensor([10 for i in range(len(samples))], dtype=torch.long)
        prompt_end_pos = torch.tensor([samples[i]["prompt_length"] - 4 for i in range(len(samples))], dtype=torch.long)
        ...
        #     "disgusted": 6,
        # }
        # emo_ids = torch.tensor(
        #     [EMO2ID.get(s.get("emotion", "neutral"), 0) for s in samples],
        #     dtype=torch.long
        # )
        # print("emo_ids=", emo_ids)
        ...
        return {
            ...
            "modality_mask": modality_mask,
            # 添加代码
            "keys": keys,                     # list[str]，不需要 .to(device)
            "prompt_str_pos": prompt_str_pos,
            "prompt_end_pos": prompt_end_pos, # Tensor [B]，会被 train_utils 挪到 GPU
        }
