...
class slam_model_tts(slam_model):
    def __init__(
        ...
        **kwargs,
        ...
        # 添加代码
        self.align_config = getattr(train_config, "align", None)
        if self.align_config is not None and self.align_config.enable:
            # 尽量从 llm.config 取 hidden_size
            hidden_size = getattr(self.llm.config, "hidden_size", None)
            if hidden_size is None:
                hidden_size = getattr(self.llm.model.config, "hidden_size", None)
            self.align_text_proj = torch.nn.Linear(hidden_size, self.align_config.proj_dim, bias=False)
            # ===== 新增：learnable logit_scale（标量参数）=====
            if getattr(self.align_config, "learnable_logit_scale", True):
                init = math.log(1.0 / float(getattr(self.align_config, "temperature", 0.07)))
                self.align_logit_scale = torch.nn.Parameter(torch.ones([]) * init)
            else:
                self.align_logit_scale = None
        else:
    ...
    def forward(
        ...
        **kwargs,
        ...
        if self.train_config.modeling_paradigm == "serial":
            ...
            need_hs = (self.align_text_proj is not None) and (kwargs.get("prompt_end_pos", None) is not None)
        ...
        # 添加代码
        # 用最后一个token
        # prompt_end_pos = kwargs.get("prompt_end_pos", None)
        # text_z = None
        # if need_hs:
        #     hs = model_outputs.hidden_states[-1]  # [B, T, H]
        #     B, T, H = hs.shape
        #     end_pos = prompt_end_pos.clamp(min=0, max=T-1)
        #     text_h = hs[torch.arange(B, device=hs.device), end_pos]  # [B, H]
        #     text_z = self.align_text_proj(text_h)                # [B, 1024]
        #     if self.align_config.normalize:
        #         text_z = F.normalize(text_z, dim=-1)
        #     # 挂到输出上，train_utils 直接取
        #     setattr(model_outputs, "text_style_emb", text_z)
        # 用平均token
        prompt_str_pos = kwargs.get("prompt_str_pos", None)
        prompt_end_pos = kwargs.get("prompt_end_pos", None)
        text_z = None
        if need_hs:
            hs = model_outputs.hidden_states[-1]  # [B, T, H]
            ...
            device = hs.device
            # 1) clamp 到合法范围
            str_pos = prompt_str_pos.clamp(min=0, max=T - 1)  # [B]
            end_pos = prompt_end_pos.clamp(min=0, max=T - 1)  # [B]
            # 2) 防止出现 end < start（否则长度为负）
            end_pos = torch.maximum(end_pos, str_pos)
            # 3) prefix sum: prefix[:, t] = hs[:, :t+1] 的累加和
            prefix = hs.cumsum(dim=1)  # [B, T, H]
            batch_idx = torch.arange(B, device=device)
            # 区间和 = prefix[end] - prefix[start-1] (若 start=0，则减 0)
            end_sum = prefix[batch_idx, end_pos]  # [B, H]
            start_m1 = (str_pos - 1).clamp(min=0)  # [B]
            start_sum = prefix[batch_idx, start_m1]  # [B, H]
            seg_sum = end_sum - torch.where(str_pos.unsqueeze(-1) > 0, start_sum, torch.zeros_like(start_sum))  # [B, H]
            # 4) 区间长度（包含 end）
            seg_len = (end_pos - str_pos + 1).clamp(min=1)  # [B]
            text_h = seg_sum / seg_len.unsqueeze(-1)  # [B, H]
            # 5) 投影 + 可选归一化
            text_z = self.align_text_proj(text_h)  # [B, 1024]
            if self.align_config.normalize:
                text_z = F.normalize(text_z, dim=-1)
        ...
        return model_outputs, text_acc, audio_acc, loss_recorder, text_z
