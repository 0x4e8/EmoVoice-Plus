...
import uuid
import logging
from utils.emosteer_utils import emosteer_context
...
def audio_decode_cosyvoice(audio_tokens, model_config, codec_decoder, audio_prompt_path=None, code_layer=1, num_latency_tokens=1, speed=1.0, replace_token=4095, decode_config = None, emoSteerEnable=False):
    """
    ...
    # # 修改代码
    # # ===== EmoSteer: attach hooks on estimator =====
    # estimator = codec_decoder.model.flow.decoder.estimator  # v1/v2 都有（不开 TRT）:contentReference[oaicite:6]{index=6}
    # print("AAAAAAAAAAAA-----\n", estimator)
    # # 构建要输出的完整字符串（精确复制 print 的格式）
    # output_content = "AAAAAAAAAAAA-----\n " + str(estimator) + "\n"  # 注意：原 print 会在参数间自动添加空格
    # # 1. 打印到控制台（保持原行为）
    # print(output_content, end='')  # end='' 避免额外换行
    # # 2. 写入到文本文件
    # with open('/data/home/zdhs0047/zdhs0047_src_data/zengfanshuo/estimator.txt', 'w', encoding='utf-8') as f:
    #     f.write(output_content)
    # if emosteer_cfg and emosteer_cfg.get("enable", False):
    #     vecs = load_vecs(emosteer_cfg["vec_path"])
    #     steerer = EmoSteerer(
    #         vecs=vecs,
    #         alpha=float(emosteer_cfg.get("alpha", 0.6)),
    #         layers=emosteer_cfg.get("layers", None),
    #         mode=emosteer_cfg.get("mode", "add"),
    #         steer_only_cond_branch=bool(emosteer_cfg.get("steer_only_cond_branch", True)),
    #     )
    #     with steerer.attach(estimator):
    ...
    #         raise NotImplementedError
    if emoSteerEnable:
        print("AAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAA")
        ...
        print("BBBBBBBBBBBBBBBBBBBBBBBBBBBBBBBBBBBBBBBBBBBBBBBBBBBBBBBBBBBBBBBBBBBBBBBBBBBBBBBBBBBBBBBBBBBBBBBBBBBBBBBBBBBBBBBBBBBBBBBBBBBBBBBBBBBBBBBBBBBBBBBBBBBBBBBBBBBBBBBBBBBBBBBBBBBBBBBBBBBBBBBBBBBBB")
        # Convert tokens to audio waveform
        if model_config.cosyvoice_version==1:
            audio_hat = codec_decoder.model.token2wav(
                token=audio_tokens,
